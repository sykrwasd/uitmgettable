...
const Timetable: React.FC<TimetableProps> = ({ selectedClasses, onRemoveClass }) => {
  ...
  return (
    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
      <h2 className="text-2xl font-bold text-gray-700 text-center mb-6">Your Timetable</h2>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-blue-600 text-white">
                  ...
                  {days.map(day => (
                    <th key={day} className="border border-gray-300 p-3 text-center font-semibold min-w-[180px]">
                      {day}
                    </th>
                  ))}
                </tr>
                ...
                {timeSlots.map(timeSlot => (
                  <tr key={timeSlot} className="hover:bg-gray-50">
                    ...
                    {days.map(day => {
                      ...
                      const classInTimeRange = !classInSlot ? selectedClasses.find(cls => {
                        if (cls.day !== day) return false;
                        
                        // Check if the class time overlaps with current slot
                        const [slotStart, slotEnd] = timeSlot.split('-');
                        const classTimeMatch = cls.timeSlot.match(/(\d{2}):(\d{2})-(\d{2}):(\d{2})/);
                      ...
                      return (
                        <td key={`${day}-${timeSlot}`} className="border border-gray-300 p-2 h-20 relative">
                          {displayClass && (
                            <div 
                              className="bg-blue-500 text-white p-2 rounded text-xs h-full cursor-pointer hover:bg-blue-600 transition-colors"
                              onClick={() => onRemoveClass(displayClass.class_code, displayClass.day_time)}
                              title="Click to remove"
                            >
                              <div className="font-semibold">{displayClass.class_code}</div>
                              <div className="text-xs opacity-90">{displayClass.venue}</div>
                              <div className="text-xs opacity-75">{displayClass.subject_code}</div>
                              <div className="text-xs opacity-60">{displayClass.timeSlot}</div>
                            </div>
                          )}
                        </td>
                      );
        ...
        {selectedClasses.length > 0 && (
          <div className="mt-6 bg-white/80 backdrop-blur-sm rounded-lg p-4">
            <h3 className="text-lg font-semibold text-gray-700 mb-3">Selected Classes ({selectedClasses.length})</h3>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              {selectedClasses.map((cls, idx) => (
                <div key={idx} className="bg-blue-50 border border-blue-200 rounded p-3">
                  <div className="font-medium text-blue-900">{cls.class_code}</div>
        ...
        {selectedClasses.length === 0 && (
          <div className="mt-6 text-center py-8">
            ...
            <p className="text-gray-400 text-sm mt-2">Select classes from the left panel to see them in your timetable</p>
          </div>
        )}
    </div>
  );
